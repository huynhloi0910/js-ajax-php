	...
	if (isset($_POST['register'])) {
        ...
        if ($check) {
        	...
        	if (move_uploaded_file($_FILES["avatar"]["tmp_name"], './uploads/'.$_FILES["avatar"]["name"])) {
                echo "File ". $_FILES["avatar"]["name"]." Đã upload thành công";
            } else {
            	echo "Không upload được file!";
            }

        	$sql = "INSERT INTO user 
                    (name, email, pass, city, phone, avatar) 
                    VALUES ('".$name."', '".$email."', '".$pass."', '".$city."', '".$phone."', '".$img."');";
		    
		    //Chạy câu SQL
		    if ($con->query($sql)) {
		        echo "<h1>Đăng Ký thành công Click vào <a href='login.php'>đây</a> để Đăng nhập </h1>";
		    }else{
				...
				<div class="col-sm-4">
					<div class="signup-form"><!--sign up form-->
						...
						<form method="POST" enctype="multipart/form-data">
							...
							<input type="email" placeholder="Email Address" name="email" value="<?php echo isset($email) ? $email : ''; ?>" >
							...
							<input type="text" placeholder="City" name="city"  value="<?php echo isset($city) ? $city : ''; ?>" />
							<?php echo  isset($errorCity) ? $errorCity : '' ?>

							<input type="text" placeholder="Phone" name="phone" value="<?php echo isset($phone) ? $phone : ''; ?>" />
							<?php echo  isset($errorPhone) ? $errorPhone : '' ?>

							<input type="file" name="avatar" value="<?php echo isset($img) ? $img : ''; ?>">
							<?php echo  isset($errorImg) ? $errorImg : '' ?>
